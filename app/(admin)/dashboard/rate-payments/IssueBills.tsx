...
export function IssueBills({ year, isOwner, onIssued }: { year: number; isOwner: boolean; onIssued: () => void }) {
  const [open, setOpen] = useState(false);
  const [counties, setCounties] = useState<{ county_id: string; name: string }[]>([]);
  const [county, setCounty] = useState('');
  const [ratingYear, setRatingYear] = useState(year);
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className={BUTTON}>
        <FilePlus2 className="h-4 w-4" /> Issue bills
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setOpen(false)}>
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-base font-semibold text-text-primary">Issue rate bills</h2>
            ...
            {preview && (
              <div className="border-border-default bg-hover-surface mt-4 border-[0.5px] p-3 text-sm text-text-primary">
                <p className="font-medium">
                  {preview.county} {ratingYear}: {preview.parcels} owned parcel{preview.parcels === 1 ? '' : 's'}, {formatMoney(preview.total_billed)} in total
                </p>
                <p className="mt-1 text-xs text-text-secondary">
                  {preview.created} new bill{preview.created === 1 ? '' : 's'} · {preview.updated} unpaid bill{preview.updated === 1 ? '' : 's'} updated · {preview.unchanged} unchanged or already paid
                </p>
              </div>
            )}
